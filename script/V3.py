...
class Chatbot:
    ...
    def add_to_conversation(
        ...
        while True:
            if len(self.conversation[convo_id]) > 10:
                self.conversation[convo_id].pop(0)
            else:
                break
    ...
    def __truncate_conversation_file(self,file,convo_id):
        """
        Truncate the conversation file
        """
        print("truncate the file convo_id:",convo_id)
        json_data = {}
        with open(file,"r",encoding="utf-8") as json_file:
            json_data = json.load(json_file)
            print("file len:",len(json_data[convo_id]))
            while True:
                if ( len(json_data[convo_id]) > 40):
                     #  limit 20 peer conversations
                    json_data[convo_id].pop(0)
                else:
                    break
        print("file after len:",len(json_data[convo_id]))
    ...
    def save(self, file: str, *convo_ids: str) -> bool:
        ...
        """
        try:
            print("convo_id:",convo_ids[0])
            if convo_ids and os.path.exists(file):
                convo_id = convo_ids[0]
                json_data = {}
                with open(file, "r", encoding="utf-8") as f:
                    json_data = json.load(f)
                    if convo_id not in json_data : json_data = {convo_id:[]}
                    # print("json data -2:",self.conversation[convo_id][-2:])
                    json_data[convo_id].extend(k for k in self.conversation[convo_id][-2:])
            ...
            # 截取文件
            self.__truncate_conversation_file(file,convo_ids[0])    
    ...
    def truncate_log(self,file):
         """
          truncate conversation log
         """
         with open(file, "w", encoding="utf-8") as f:
            f.truncate(0)
    ...
    def load_log(self,num,file,convo_id):
        """
          load log of converation
        """
        result = []
        with open(file,'r',encoding='utf-8') as f:
            f_d = json.load(f)
            data = f_d[convo_id] if len(f_d)>0 else f_d
            if len(data)> num:
                result = data[-num:]
            else:
                result = data
